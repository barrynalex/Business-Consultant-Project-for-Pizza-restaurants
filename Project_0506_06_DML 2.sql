...
--3.What is the review distribution based on gender?
...
SELECT CASE WHEN ravg.avgRating >= 4 THEN 'YES' ELSE 'NO' END AS 'Rating >= 4', r.rstId, AVG(r.rstCapacity) AS 'Capacity'
FROM [Master_Lee's.Restaurant] r, [Master_Lee's.Review] rv, rstAvgRating ravg
WHERE r.rstId = rv.rstId AND r.rstId = ravg.rstId
GROUP BY r.rstId, ravg.avgRating
UNION
SELECT CASE WHEN ravg.avgRating >= 4 THEN 'YES' ELSE 'NO' END AS 'Rating >= 4', 'Average Capacity', AVG(r.rstCapacity) AS 'Capacity'
FROM [Master_Lee's.Restaurant] r, [Master_Lee's.Review] rv, rstAvgRating ravg
WHERE r.rstId = rv.rstId AND r.rstId = ravg.rstId
...
-- Are there any customers that comments multiple times?
SELECT c1.cstId, c1.cstName
FROM [Master_Lee's.Customer] c1, cstReviewCount c2
WHERE c1.cstId = c2.cstId
  AND c2.reviewCount > 1
